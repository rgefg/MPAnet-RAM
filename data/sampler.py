...
class CrossModalityRandomSampler(Sampler):
    ...
    def __iter__(self):
        ...
        if rgb_size >= ir_size:
            ...
            for _ in range(reps):
                ir_list.extend(np.random.permutation(self.ir_list).tolist())          #随机排列复制
            ir_list.extend(np.random.choice(self.ir_list, pad_size, replace=False).tolist())           #补余，通过扩展机制让 RGB/IR 数量一致
        else:
        ...
        half_bs = self.batch_size // 2                    #取半个batchsize
...
class CrossModalityIdentitySampler(Sampler):                #在每个 batch 中采样若干个 identity，每个 identity 从 RGB 和 IR 模态各采 k 个样本（PK 结构）
